...
class SyntheticDataLoader:
    def get_data(self, n_days, n_drifts, start_price, seed):
        n_days -= n_days % (n_drifts + 1)
        np.random.seed(seed)
        ...
        for i in range(n_drifts + 1):
            mu = np.random.uniform(0.0005, 0.002) if i % 2 == 0 else np.random.uniform(-0.002, -0.0005) 
        ...
        highs = np.maximum(opens, closes) * (1 + high_spreads)
        lows = np.minimum(opens, closes) * (1 - low_spreads)
        ...
        self.raw_data = df.dropna().reset_index(drop=True)
    ...
    def pipeline(self, n_days=1000, n_drifts=3, start_price=100.0, seed=42):
        self.get_data(n_days, n_drifts, start_price, seed)
        self.add_features_and_target()
        
        return self.data_with_target, self.regimes[:len(self.data_with_target)]
